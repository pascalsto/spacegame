...
body_speed[0] = np.array([0, 2])
body_acc[0] = np.array([0, 0])

# Body 1 Anfangsbedingungen
body[1] = np.array([WIDTH // 2 - 200, HEIGHT // 2])
body_speed[1] = np.array([0, 2])
body_acc[1] = np.array([0, 0])

# Body 2 Anfangsbedingungen
body[2] = np.array([WIDTH // 2 + 200, HEIGHT // 2])
body_speed[2] = np.array([0, -2])
...
while True:
    ...
    for i in range(len(body)):              # Berechnung der Beschleunigung
        for j in range(len(body)):
            if i == j:
                break
            distance = np.linalg.norm(body[j] - body[i])
            if distance < 1:
                distance = 1
            body_acc[i] += (G * body_mass[j] * (body[j] - body[i]))/(distance**3)
    ...
    for i in range(len(body)):
        pygame.draw.circle(screen, BODY_COLOR[i], body[i].astype(int), body_radius[i])
